/* qfcl/random/distribution/boost-trunk/normal_distribution.hpp 
 *
...
#ifndef QFCL_BOOST_RANDOM_NORMAL_DISTRIBUTION_HPP
#define QFCL_BOOST_RANDOM_NORMAL_DISTRIBUTION_HPP

/*! \file qfcl/random/distribution/boost-trunk/normal_distribution.hpp
	\brief univariate normal distribution using the Ziggurat method.

	\author Jens Maurer, Steven Watanabe, James Hirschorn
	\date May 18, 2014
*/
...
template<class RealType, std::size_t w, class Engine, class U01_Dist>
inline std::pair<RealType, int> generate_int_float_pair(Engine& eng, boost::mpl::false_)
{
    ...
    RealType r = U01_Dist<RealType>()(eng);
    return std::make_pair(r, bucket);
}
...
template<class RealType = double, class U01_Dist = boost::uniform_01<RealType>>
struct unit_normal_distribution
{
    ...
    RealType operator()(Engine& eng) {
        ...
        for(;;) {
            ...
            RealType y = RealType(table_y[i]) + U01_Dist()(eng) * RealType(table_y[i + 1] - table_y[i]);
...
#endif // QFCL_BOOST_RANDOM_NORMAL_DISTRIBUTION_HPP
